...
/**
  * A schema import resolver that uses the classpath to look for schema definitions.
  *
  * @param loader the underlying resource loader
  */
class ClasspathResolver[F[_]](loader: ResourceLoader[F])(implicit F: MonadError[F, Throwable])
  ...
  private def load(uri: String): F[ShaclSchema] =
    if (uri.startsWith(loader.baseUri.toString())) loader(Uri(uri)).map(json => ShaclSchema(json))
